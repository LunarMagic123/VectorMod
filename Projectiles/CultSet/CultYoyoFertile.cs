...
{
	public class CultYoyoFertile : ModProjectile
	{
		...
		public override void AI() {
			...
			if ((Main.MouseWorld - player.Center).Length() <= (player.yoyoString ? 450 : 575))
            {
				targetPos = Main.MouseWorld;
            }
			else
            {
				targetPos = player.Center + Vector2.Normalize(Main.MouseWorld - player.Center) * (player.yoyoString ? 450 : 575);
            }
			...
			if (player.channel)
			{
				...
				if (bitchTimer < 25)
				{
					projectile.velocity += Vector2.Normalize(targetPos - projectile.Center) * 2 * player.meleeSpeed;
				}
				else
				{
					if ((Main.MouseWorld - player.Center).Length() <= 50)
					{
						projectile.velocity += Vector2.Normalize(targetPos - projectile.Center) * 5 * player.meleeSpeed;
					}
					else
					{
						projectile.velocity = (targetPos - projectile.Center) / 20;
					}
				}
			}
			else
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
			...
			if (player.yoyoGlove && player.ownedProjectileCounts[ProjectileType<CultYoyoSterile>()] == 0)
			{
				Projectile.NewProjectile(player.Center, Vector2.Zero, ProjectileType<CultYoyoSterile>(), projectile.damage, projectile.knockBack, Main.myPlayer);
			}
		}
    }
}
