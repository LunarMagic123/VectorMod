...
{
	public class StarSlicerBrown : ModProjectile
	{
		...
		public override void AI()
		{
			...
			for (int i = 0; i < Main.maxNPCs; i++)
            {
				...
				if (npcScan.CanBeChasedBy() && Collision.CanHitLine(projectile.Center, 0, 0, npcScan.Center, 0, 0) && (npcScan.Center - player.Center).Length() < 400)
                {
					hasTarget = true;
			...
			if (hasTarget)
			{
				if ((targetPos - projectile.Center).Length() > 75)
				{
					projectile.velocity += Vector2.Normalize(targetPos - projectile.Center) * 3;
				}
				if (projectile.velocity.Length() >= 22.5f)
				{
					projectile.velocity = Vector2.Normalize(projectile.velocity) * 22.5f;
				}
				for (int j = 1; j < 3; j++)
				{
					Dust dust = Dust.NewDustPerfect(projectile.Center, DustType<StardustBrown>(), Vector2.Normalize(projectile.velocity) * j * 5, 100, default, 1.5f);
					dust.noGravity = true;
				}
			}
			else
